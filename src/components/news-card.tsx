...
const categoryStyles: { [key: string]: string } = {
  ...
  General: 'border-gray-400/40 bg-gray-500/10 text-gray-400',
};
...
export function NewsCard({ article }: { article: Article }) {
  ...
  const dateString = formatDistanceToNow(date, { addSuffix: true });
  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden transition-shadow duration-300 hover:shadow-xl hover:-translate-y-1 rounded-lg">
      ...
      <CardContent className="flex-grow p-4">
        <Badge variant="outline" className={cn("mb-2", categoryStyles[article.category] || categoryStyles.General)}>{article.category}</Badge>
